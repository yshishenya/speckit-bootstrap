...
- Настройки автокоммитов документации сохраняются, включая явное отключение.
  Bootstrap не включает их при обновлении. Если они включены в
  `.specify/extensions/git/git-config.yml`, обработчики `after_constitution`,
  `after_specify`, `after_clarify`, `after_plan`, `after_checklist`, `after_tasks`
  и `after_analyze` могут фиксировать документы своей команды после проверки
  области изменений и с соблюдением правил подтверждения проекта.
...
- Реализация идёт небольшими проверяемыми сценариями: целевой падающий тест,
  соответствующее изменение, затем затронутые проверки. Предварительная уборка
  кода нужна только там, где облегчает это изменение. Приёмочные и выпускные
  проверки сохраняются.
- Продолжайте с актуальными spec/plan/tasks и результатами проверок, не создавая
  пройденные этапы заново. Изменившиеся входы, включая незакоммиченные правки,
  требуют перепроверки; совпадения SHA недостаточно.
- Настройки `context_file` и `context_files` в
  `.specify/extensions/agent-context/agent-context-config.yml` сохраняются.
  Отдельный файл контекста позволяет убрать указатель текущей функции из
  постоянно читаемого `AGENTS.md`; общие правила безопасности остаются в цепочке
  инструкций. Пустые значения используют стандартный выбор расширения.
