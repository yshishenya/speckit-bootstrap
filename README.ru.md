...
- **Официальный Spec Kit с pin.** Последний стабильный тег `vMAJOR.MINOR.PATCH` разрешается в
...
Перед обновлением проекта со схемой lock v3 проверяются все управляемые навыки.
Только правки bootstrap, совпадающие с lock-файлом, согласуются с контрольными
суммами установки Codex. Обновление не требует `--force` и сохраняет изменённые
общие скрипты и шаблоны проекта. Пользовательские изменения навыков останавливают
процесс до смены CLI, workflow и расширений. В режиме `--frozen` такого
согласования нет. По умолчанию выбирается стабильный тег, без `alpha`, `beta`
и `rc`; явное указание версии или ref остаётся доступным.

Spec Kit 1.0.6 поддерживается без ослабления проверок: нечитаемый YAML обработчиков
по-прежнему блокирует работу, а сведения об авторе расширения сохраняются.
...
| --- | --- |
| `SPEC_KIT_VERSION` | Тег или ref Spec Kit; по умолчанию последний стабильный тег `vMAJOR.MINOR.PATCH` |
