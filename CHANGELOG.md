...
- Обновление больше не возвращает контекст в `AGENTS.md` поверх выбранного
  `context_file`/`context_files` и не включает отключённые автокоммиты документов.
  Явно включённые документальные обработчики сохраняются; автоматические
  коммиты реализации и предварительные обработчики остаются отключёнными.
- Обновление настроек согласует контрольные суммы неизменённых управляемых
  навыков с манифестом Codex. Собственные правки bootstrap больше не требуют
  `--force`; пользовательские изменения по-прежнему блокируют перезапись.

### Изменено

- Сокращены управляемые инструкции GitHub и Ponytail без удаления обязательных
  проверок и правил закрытия задач.
- Генератор инструкций реализации использует небольшие проверяемые сценарии,
  условную предварительную уборку кода и продолжение с актуальными артефактами.
  Новые команды, зависимости и изменения официального upstream Spec Kit не нужны.
