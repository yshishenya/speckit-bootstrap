...
- Восстановлена обязательная проверка завершённости после converge в генерируемом workflow GRAF; обновляется и ранее установленный вариант.

- Создание ветки и спецификации использует один номер из распределителя проекта и сохраняет полные метаданные фичи.
- Явные номера, временные метки и режим пропуска резервирования не обходят политику проекта. Старый путь создания только каталога останавливается до записи в проектах с распределителем.
- Повторная генерация сохраняет эти правила; общий режим проектов без распределителя остаётся доступным.

- Совместимость с Spec Kit 1.0.6: новый текст предупреждения о нечитаемом YAML
  переводится в обязательную остановку; смешанные неизвестные варианты отклоняются.
  Сохраняется новое поле автора в проверенном навыке контекста GRAF.
- Обновление навыков Codex, ранее изменённых самим bootstrap: перед обновлением
  проверяется весь набор навыков по lock-файлу и согласуются только известные
  контрольные суммы установки. Пользовательские изменения, посторонние пути и
  символьные ссылки не принимаются; принудительная перезапись общих файлов не нужна.
- Сохранён проверенный навык GRAF с выбором активного плана только через указатель,
  без возврата к выбору по времени изменения файлов.
- Проектный workflow GRAF принимает структуру Spec Kit 1.0.5 и сохраняет полный
  цикл, включая clarify, checklist, analyze, converge и завершение трекера.
- Выбор `latest` исключает предварительные версии. Явные версии/ref и `--frozen`
  сохраняют прежнее назначение.
- Добавлены проверки обновления реестра, сохранения пользовательских правок,
  структуры workflow 1.0.5 и сохранённого навыка GRAF; число тестов в отчёте
  вычисляется из фактически выполненных проверок.
...
- Автокоммиты завершённых документальных этапов выполняются без повторного
  запроса подтверждения после проверки области изменений.
