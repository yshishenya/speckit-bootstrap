...
test_ponytail_opt_out_removes_project_guidance() (
  local sandbox="$TEST_ROOT/ponytail-opt-out"
  PROJECT_DIR="$sandbox/project"
  mkdir -p "$PROJECT_DIR/docs/agent-guidance"
  printf '# Project\n\n<!-- SPECKIT PONYTAIL START -->\nold guidance\n<!-- SPECKIT PONYTAIL END -->\n' \
    > "$PROJECT_DIR/AGENTS.md"
  printf "# Ponytail Upstream AGENTS.md\n\nGenerated by \`speckit-bootstrap\` from the installed Ponytail plugin cache.\n" \
    > "$PROJECT_DIR/$PONYTAIL_UPSTREAM_AGENTS_PATH"

  remove_ponytail_project_guidance

  if grep -Fq 'SPECKIT PONYTAIL START' "$PROJECT_DIR/AGENTS.md"; then
    return 1
  fi
  [[ ! -e "$PROJECT_DIR/$PONYTAIL_UPSTREAM_AGENTS_PATH" ]]
...
for marker in (
    ...
    "after_implement` is never covered by this",
...
run_test 'Ponytail opt-out removes project guidance' test_ponytail_opt_out_removes_project_guidance
